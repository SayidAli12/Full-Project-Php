...
join students on borrows.Student_id = students.id where borrows.status = 'pending' and time between '$date1' and '$date2'") or die(mysqli_error($mysqli));
} elseif (isset($_POST['clear'])) {
    $date1 = '';
    $date2 = '';
}

// SQL query to count rows where status is 'pending'
// SQL query to count rows where status is 'pending' and time is between date1 and date2
$sql1 = "SELECT COUNT(id) AS pending FROM borrows WHERE status = 'pending' AND time BETWEEN '$date1' AND '$date2'";
...
<head>
    <meta charset="UTF-8">
    <title>Pending Books Report</title>
                ...
                <div class="card p-1 mb-2 animate__animated animate__fadeIn">
                    <div class="btn-group">
                        ...
                        <a href="pending.php" class="btn btn-warning m-2">
                            <i class="fas fa-hourglass-half"></i>&nbsp;Pending Books&nbsp;:&nbsp;<?php echo $pending; ?>
                        </a>
        ...
        <div class="row justify-content-center">
            <div class="col">
                <div class="card animate__animated animate__fadeIn">
                    <div class="card-header bg-white text-dark">BORROWED BOOKS REPORT</div>
                    <div class="card-body">
                                ...
                                <?php if ($result && $result->num_rows > 0): ?>
                                    <?php while ($row = $result->fetch_assoc()): ?>
                                        ...
                                        if ($row['status'] == 'Pending') {
                                            ...
                                            echo "<td class='text-danger font-size-bold'>" . $row['status'] . "</td>";
                                        }
                                    ?>
                                </tr>
                                <?php endwhile; ?>
